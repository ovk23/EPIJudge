...
public class SearchMaze {
    ...
    private static boolean helper(List<List<Color>> maze,
                                  Coordinate start,
                                  Coordinate end,
        ...
        if (start.x < 0 ||
                start.x >= maze.size() ||
                start.y < 0 ||
                start.y >= maze.get(start.x).size() ||
                maze.get(start.x).get(start.y) != Color.WHITE) {
            return false;
        }
        ...
        for (Coordinate next : List.of(new Coordinate(start.x, start.y + 1),
